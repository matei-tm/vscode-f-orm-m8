...
export default function getConcreteIndependentEntityDatabaseHelper(
  version: string,
  packageName: string,
...
import 'package:${packageName}/models/independent/${entityNameInUnderscoreCase}.dart';

mixin ${entityNameInPascalCase}DatabaseHelper implements AbstractDatabaseHelper {
  Future<Database> db;
  final String columnId = 'id';
  final ${entityNameInCamelCase}Columns = ["id", "record_date", "is_deleted" /*, "entry_name"*/];
  ...
  Future create${entityNameInPascalCase}Table(Database db) async {
    await db.execute(
        'CREATE TABLE $_${entityNameInCamelCase}Table ($columnId INTEGER PRIMARY KEY, record_date INTEGER, is_deleted INTEGER ${tableColumnsDefinitionConcatenation})');
  }
